...
from models import UserProfile, ChirieRemoteSigning, utcnow

from .form import ChirieForm
from pdf.chirie_pdf import render_chirie_pdf_bytes


chirie_bp = Blueprint("chirie", __name__, url_prefix="/chirie")
...
def get_tmp_dir() -> Path:
    # IMPORTANT: absolut, ca să nu depindă de "working directory"
...
def _profile_to_dict(p: UserProfile) -> dict:
    """Compatibil cu template/PDF existente: profile.agent.name, profile.agency.name etc."""
    return {
        "agent": {
...
def get_or_create_profile() -> UserProfile:
    """Profil separat per user (din DB). Dacă nu există, îl creează."""
...
def cleanup_expired_remote():
    """
    Șterge automat remote-urile expirate:
    - DB record
    ...
    expired = (
        ChirieRemoteSigning.query
        .filter(ChirieRemoteSigning.expires_at.isnot(None))
        .filter(ChirieRemoteSigning.expires_at < now)
        .all()
    )
...
def _token() -> str:
    # token public greu de ghicit
    return secrets.token_urlsafe(32)  # ~43 chars


def _default_expiry():
    # ai setat 3 zile - las așa cum e la tine
    return utcnow() + timedelta(days=3)


# -----------------------------
# NEW: semnare meta (doar pt PDF, fără DB)
# -----------------------------
def _get_client_ip(req) -> str:
    """
    IP real în producție (proxy-aware):
    - X-Forwarded-For (primul IP)
    - X-Real-IP
    - remote_addr
    """
...
# ============================================================
# VARIANTA 1 — Actuală (on-site)
# ============================================================

@chirie_bp.route("/preview", methods=["POST"])
@login_required
@agent_required
@access_required
def preview_pdf():
    ...
    payload = {
        ...
        "vizitator": {
            ...
            # CI (dacă există în form)
            "ci_serie": _safe_strip(getattr(form, "ci_serie", None).data) if getattr(form, "ci_serie", None) else "",
            "ci_numar": _safe_strip(getattr(form, "ci_numar", None).data) if getattr(form, "ci_numar", None) else "",
        },
        "imobil": {
    ...
    pdf_bytes = render_chirie_pdf_bytes(payload)

    tmp_dir = get_tmp_dir()
    doc_id = uuid.uuid4().hex[:12]
    pdf_path = tmp_dir / f"preview-{doc_id}.pdf"
    ...
    resp = send_file(
        ...
        download_name="preview-fisa-vizionare-chirie.pdf",
        conditional=True,
        max_age=0,
    )
...
@chirie_bp.route("/", methods=["GET", "POST"])
@login_required
@agent_required
@access_required
def form_page():
    ...
    if request.method == "GET" and not current_user.can_create_rental_viewing():
    ...
    if form.validate_on_submit():
        ...
        # CI fields (dacă nu sunt în WTForms încă)
        ...
        # Marchează că a folosit accesul gratuit pentru fișă chirie (doar dacă nu e plătit și nu a folosit deja)
        # Permitem generarea PDF-ului chiar dacă a folosit deja accesul gratuit, dar marcăm doar dacă nu a folosit încă
        if current_user.can_create_rental_viewing():
            current_user.mark_rental_viewing_used()
            db.session.commit()

        return redirect(url_for("chirie.done_page", doc_id=doc_id))

    if request.method == "POST":
        flash("Completează câmpurile obligatorii.", "error")

    return render_template("chirie_form.html", form=form, show_activate_modal=show_activate_modal_on_load)


@chirie_bp.route("/done/<doc_id>")
@login_required
@agent_required
@access_required
def done_page(doc_id):
    ...
    if not meta_path.exists():
        ...
        return redirect(url_for("chirie.form_page"))

    meta = json.loads(meta_path.read_text(encoding="utf-8"))
    return render_template("chirie_done.html", doc_id=doc_id, meta=meta)


@chirie_bp.route("/pdf/<doc_id>")
@login_required
@agent_required
def download_pdf(doc_id):
    ...
    response.headers["Content-Disposition"] = 'attachment; filename="fisa-vizionare-chirie.pdf"'
    response.headers["X-Content-Type-Options"] = "nosniff"
    response.headers["Cache-Control"] = "no-store"
    return response


@chirie_bp.route("/view/<doc_id>")
@login_required
@agent_required
@access_required
def view_pdf(doc_id):
...
# ============================================================
# VARIANTA 2 — Remote signing
# ============================================================

@chirie_bp.route("/remote", methods=["GET"])
@login_required
@agent_required
@access_required
def remote_agent_page():
    p = get_or_create_profile()
    profile = _profile_to_dict(p)
    return render_template("chirie_remote_agent.html", profile=profile)


@chirie_bp.route("/remote/create", methods=["POST"])
@login_required
@agent_required
@access_required
def remote_create():
    # Verifică dacă user neplătit poate crea fișă chirie (trial pe acțiuni)
    if not current_user.can_create_rental_viewing():
        return jsonify({
            "ok": False,
            "error": "Acces limitat atins",
            "message": "Ai folosit deja accesul gratuit pentru fișă de vizionare chirie. Activează abonamentul ca să continui.",
    ...
    adresa_full = (request.form.get("adresa_locuintei") or "").strip()
    ...
    rec = ChirieRemoteSigning(
        ...
        signature_agent_dataurl=sig_agent or None,
    )
    db.session.add(rec)
    
    # Marchează că a folosit accesul gratuit pentru fișă chirie (doar dacă nu e plătit)
    current_user.mark_rental_viewing_used()
    db.session.commit()

    link = url_for("chirie.remote_public_page", token=rec.public_token, _external=True)
    return jsonify({"ok": True, "link": link})


@chirie_bp.route("/s/<token>", methods=["GET"])
def remote_public_page(token):
    cleanup_expired_remote()

    rec = ChirieRemoteSigning.query.filter_by(public_token=token).first()
    if not rec:
        return render_template("remote_invalid.html", message="Link invalid."), 404
...
@chirie_bp.route("/s/<token>/submit", methods=["POST"])
def remote_submit(token):
    ...
    if not viz_nume or not sig_visitor:
        return redirect(url_for("chirie.remote_public_page", token=token))
    ...
    # moment semnare (RO) + IP real
    signed_local = _signed_at_ro_str()
    ...
    payload = {
        ...
        # meta remote (doar în PDF)
        "signature_meta": {
            "mode": "remote",
            "signed_at_local": signed_local,
            "timezone": "Europe/Bucharest",
            "ip": ip,
    ...
    tmp_dir = get_tmp_dir()
    doc_id = uuid.uuid4().hex[:12]
    pdf_path = tmp_dir / f"remote-{doc_id}.pdf"
    pdf_path.write_bytes(pdf_bytes)

    # DB: păstrezi doar ce aveai (fără IP)
    ...
    rec.pdf_doc_id = f"remote-{doc_id}"
    db.session.commit()
...
from models import ChirieRemoteSigning, VanzareRemoteSigning  # adaugă VanzareRemoteSigning

@chirie_bp.route("/remote/list", methods=["GET"])
@login_required
@agent_required
@access_required
def remote_list():
    cleanup_expired_remote()  # chirie cleanup

    chirii = (
        ChirieRemoteSigning.query
        .filter_by(user_id=current_user.id)
        .order_by(ChirieRemoteSigning.created_at.desc())
        .limit(300)
        .all()
    )

    # dacă ai cleanup separat pentru vanzare, îl chemăm aici când îl facem
    vanzari = (
        VanzareRemoteSigning.query
        .filter_by(user_id=current_user.id)
        .order_by(VanzareRemoteSigning.created_at.desc())
        .limit(300)
        .all()
    )

    return render_template("chirie_remote_list.html", chirii=chirii, vanzari=vanzari)



@chirie_bp.route("/remote/pdf/<token>", methods=["GET"])
@login_required
@agent_required
@access_required
def remote_download_pdf(token):
    ...
    rec = ChirieRemoteSigning.query.filter_by(public_token=token, user_id=current_user.id).first()
